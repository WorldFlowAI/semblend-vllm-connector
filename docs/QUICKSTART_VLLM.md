...
| `capture_policy` | which requests become donors: `all` (default, every eligible request), `sampled` at `capture_sample_rate` keyed on the request id so a rerun captures the same set, or `hinted` — only requests a router marked, through `"vllm_xargs": {"semblend_capture": true}`, an `x-semblend-capture` header, or a `semblend_capture` request attribute. Capture is paid by the request being captured (a measured 1.2 GB host copy at 21.3K tokens), so a deployment whose donors are known in advance stops paying it on the rest of its traffic |
| `capture_write_queue_depth` | how many donor writes may be queued for the connector's writer thread before the forward pass has to wait for it (default 64). The write is overlapped with the rest of the forward pass rather than removed from the step — what the writer has not finished is paid at the `wait_for_save` join and reported as `flush_ms`. A full queue means the store cannot keep up, and the wait is counted rather than answered by dropping a donor |
| `capture_write_close_timeout_s` | how long teardown waits for the writer to drain, in seconds (default 5). It bounds the whole stop, so a store that has stopped responding costs this much at process exit and warns, rather than hanging |
| `donor_registration_retry_steps` | how many scheduler steps a finished donor may wait for its own capture record before it is declined for good (default 8). Steps, not calls: a batch of requests finishing inside one step spends none of the budget. vLLM frees a request before its id reaches `finished_req_ids`, so a capture still open at finish is closed by the worker one step after the registration hook ran; at 0 those donors are always lost |
| `min_semantic_span` | smallest span worth materializing. It is also a lookup gate: when fewer tokens are left after vLLM's own prefix-cache boundary than this, no span can clear the floor, so the connector skips the lookup and writes `lookup_skipped_remaining_below_min_span` instead of paying for an embedding |
| `lookup_precheck` | default `true`. In semantic-span mode, ask the provider first whether any donor in the namespace could hold an identical run from the boundary long enough to serve; an exact "no" skips the lookup and writes `lookup_skipped_no_shared_run` (needs SemBlend >= 0.3.25; otherwise the lookup runs as before) |
| `capture_async_copy` | default `true`. Copy captured layers to pinned host memory on a side CUDA stream instead of a blocking `.cpu()` per layer |
| `stage_hint_key` | default `semblend_stage`. A request carrying this hint (e.g. `vllm_xargs`) is one stage of a staged prefill: it keeps its filled blocks in vLLM's prefix cache for the next stage and is never captured as a donor |
| `multi_donor_spans` | default `false`. Serve one contiguous span assembled from several donors when the provider returns multi-donor runs (SemBlend with `SEMBLEND_MULTI_DONOR=1`); off until answer quality for assembled spans is measured |
